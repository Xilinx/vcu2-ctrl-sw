...
/*****************************************************************************/
static void writePpsRangeExtension(AL_TBitStreamLite* pBS, AL_THevcPps const* pPps)
{
  if(pPps->transform_skip_enabled_flag)
    AL_BitStreamLite_PutUE(pBS, pPps->log2_transform_skip_block_size_minus2);

  AL_BitStreamLite_PutBit(pBS, pPps->cross_component_prediction_enabled_flag);
  AL_BitStreamLite_PutBit(pBS, pPps->chroma_qp_offset_list_enabled_flag);

  AL_BitStreamLite_PutUE(pBS, pPps->log2_sao_offset_scale_luma);
  AL_BitStreamLite_PutUE(pBS, pPps->log2_sao_offset_scale_chroma);
}
...
static void writePpsData(AL_TBitStreamLite* pBS, AL_THevcPps const* pPps)
{
  ...
  if(pPps->pps_range_extension_flag)
    writePpsRangeExtension(pBS, pPps);
