...
static void XAVC_CheckCoherency(AL_TEncSettings* pSettings)
{
  ...
  if(AL_IS_INTRA_PROFILE(pChannel->eProfile))
  {
    ...
    pChannel->eEncOptions &= ~AL_OPT_L2_CACHE;
...
static void AL_sSettings_SetDefaultAVCParam(AL_TEncSettings* pSettings)
{
  ...
  pSettings->tChParam[0].eWPMode = AL_WP_IMPLICIT;
}
...
void AL_Settings_SetDefaults(AL_TEncSettings* pSettings)
{
  ...
  pChan->eEncTools |= AL_OPT_DIRECT_SPATIAL;
  pChan->eSrcMode = AL_SRC_RASTER;

  pChan->tMCParam.eMCClipMode = AL_MC_CLIP_NONE;

  pChan->eVideoMode = AL_VM_PROGRESSIVE;

  pChan->MaxNumMergeCand = 5;
  pChan->MaxNumMergeCand = -1;
...
/***************************************************************************/
static bool IsMultiChipTuned(AL_TEncChanParam* pChParam)
{
  return pChParam->tMCParam.eMCClipMode != AL_MC_CLIP_NONE || pChParam->tMCParam.uMCPosX != 0 || pChParam->tMCParam.uMCPosY != 0
         || pChParam->tMCParam.uMCWidth != 0 || pChParam->tMCParam.uMCHeight != 0;
}
...
int AL_Settings_CheckValidity(AL_TEncSettings* pSettings, AL_TEncChanParam* pChParam, FILE* pOut)
{
  ...
  if(AL_GET_CHROMA_MODE(pChParam->ePicFormat) > AL_HWConfig_Enc_GetSupportedChromaMode(pChParam->eProfile))
  {
    ...
    if(!(AL_IS_JPEG(pChParam->eProfile) && (AL_GET_CHROMA_MODE(pChParam->ePicFormat) <= AL_CHROMA_4_2_2)))
    {
      ++err;
  ...
  {
    int HStep = (pChParam->eSrcMode == AL_SRC_TILE_64x4 || pChParam->eSrcMode == AL_SRC_COMP_64x4) ? 64 :
                (pChParam->eSrcMode == AL_SRC_TILE_32x4 || pChParam->eSrcMode == AL_SRC_COMP_32x4) ? 32 :
                (pChParam->uSrcBitDepth > 8) ? (HW_IP_BURST_ALIGNMENT / sizeof(uint16_t)) : HW_IP_BURST_ALIGNMENT;
    int VStep = (pChParam->eSrcMode == AL_SRC_RASTER) ? (AL_GET_CHROMA_MODE(pChParam->ePicFormat) == AL_CHROMA_4_2_0 ? 2 : 1)
                : (AL_GET_CHROMA_MODE(pChParam->ePicFormat) == AL_CHROMA_4_2_0 ? 8 : 4);
  ...
  {
    ++err;
    MSG_ERROR("Wavefront encoding not supported by the IP");
  }
  ...
  if(pChParam->bSrcSync && (pChParam->tGopParam.uNumB > 0))
  {
    ++err;
    MSG_ERROR("B Picture not allowed with synchronised source");
  }
  ...
  if(pChParam->iL2CacheTolerance < 0 || pChParam->iL2CacheTolerance > 100)
  {
    ++err;
    MSG_ERROR("L2Cache tolerance should be in [0, 100]");
  }
  ...
  bool bMCTuned = IsMultiChipTuned(pChParam);

  if(bMCTuned)
  {
    ++err;
    MSG("Cannot tune multichip flags. Multichip encoding is configured automatically when required.");
  }
...
int AL_Settings_CheckCoherency(AL_TEncSettings* pSettings, AL_TEncChanParam* pChParam, TFourCC tFourCC, FILE* pOut)
{
  ...
  if(AL_IS_INTRA_PROFILE(pChParam->eProfile))
  {
    ...
    if(pChParam->eEncOptions & AL_OPT_L2_CACHE)
    {
      pChParam->eEncOptions &= ~AL_OPT_L2_CACHE;
      MSG_WARNING("L2 Cache shall be disabled for Intra only profile; it will be adjusted");
      ++numIncoherency;
    }
  ...
  if(!AL_IS_AVC(pChParam->eProfile) && pChParam->eWPMode != AL_WP_DEFAULT)
  {
    MSG_WARNING("Weighted Prediction is only available for AVC");
    pChParam->eWPMode = AL_WP_DEFAULT;
  }
  ...
  if(pChParam->tGopParam.eMode & AL_GOP_FLAG_PYRAMIDAL)
  {
    ...
    if(AL_IS_AVC(pChParam->eProfile) && !(pChParam->eEncTools & AL_OPT_DIRECT_SPATIAL))
    {
      pChParam->eEncTools |= AL_OPT_DIRECT_SPATIAL;
      MSG_WARNING("AVC Pyramidal GOP requires direct spatial bi-prediction, it will be adjusted");
      ++numIncoherency;
    }
  }
