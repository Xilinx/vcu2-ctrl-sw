...
static int sFrmBufPool_GetMinBufferSize(AL_TFrmBufPool const* pPool)
{
  ...
  if(pPool->bHasSecondOutputFrame)
  {
    for(int iFrameID = 0; iFrameID < pPool->tSecondOutputPool.uFrameNum; ++iFrameID)
    {
      AL_TDecFramePoolElem const* pElem = &pPool->tSecondOutputPool.tPoolElems[iFrameID];

      iMinSize = Min(iMinSize, (int)AL_Buffer_GetSize(pElem->pFrameBuf));
    }
  }
  else
  {
...
static AL_TDimension sFrmBufPool_GetMinBufferDim(AL_TFrmBufPool const* pPool, AL_EFbStorageMode eDisplayStorageMode)
{
  ...
  if(pPool->bHasSecondOutputFrame)
  {
    for(int iFrameID = 0; iFrameID < pPool->tSecondOutputPool.uFrameNum; ++iFrameID)
    {
      ...
      int const iPitchY = AL_PixMapBuffer_GetPlanePitch(pElem->pFrameBuf, AL_PLANE_Y);
      iMinPitch = Min(iMinPitch, iPitchY);

      AL_TDimension tDim = AL_PixMapBuffer_GetDimension(pElem->pFrameBuf);
...
/*************************************************************************/
bool AL_PictMngr_AllocInternalFrameBufs(AL_TPictMngrCtx* pCtx, AL_TDimension tDim, AL_EChromaMode eChromaMode, bool bEnableFBC, int iNum)
{
  AL_EPlaneMode ePlaneMode = GetInternalBufPlaneMode(eChromaMode);
  AL_TPicFormat tPicFormat = AL_GetDecPicFormat(eChromaMode, pCtx->iBitdepth, pCtx->eFbStorageMode, bEnableFBC, ePlaneMode);
