...
static map<string, EnumDescription<int>> const getOutputFourCCs(void)
{
  ...
  fourCCs["T5m8"] = { FOURCC(T5m8), "YUV file contains 4:0:0 8-bit video samples stored in 32x4 tiles", allCodecs() };
  fourCCs["T6m8"] = { FOURCC(T6m8), "YUV file contains 4:0:0 8-bit video samples stored in 64x4 tiles", allCodecs() };
  ...
  fourCCs["T508"] = { FOURCC(T508), "YUV file contains 4:2:0 8-bit video samples stored in 32x4 tiles", allCodecs() };
  fourCCs["T608"] = { FOURCC(T608), "YUV file contains 4:2:0 8-bit video samples stored in 64x4 tiles", allCodecs() };
  ...
  fourCCs["T528"] = { FOURCC(T528), "YUV file contains 4:2:2 8-bit video samples stored in 32x4 tiles", allCodecs() };
  fourCCs["T628"] = { FOURCC(T628), "YUV file contains 4:2:2 8-bit video samples stored in 64x4 tiles", allCodecs() };
  SetEnumDescr<int>(fourCCs, "NV16", FOURCC(NV16), "YUV file contains 4:2:2 8-bit video samples stored in semi-planar format with all picture luma (Y) samples followed by interleaved U and V chroma sample.", isOnlyCodec(Codec::Jpeg));
  SetEnumDescr<int>(fourCCs, "I422", FOURCC(I422), "YUV file contains 4:2:2 8-bit video samples stored in planar format with all picture luma (Y) samples followed by chroma samples (all U samples then all V samples).", isOnlyCodec(Codec::Jpeg));
  SetEnumDescr<int>(fourCCs, "YV16", FOURCC(YV16), "Same as I422.", isOnlyCodec(Codec::Jpeg));
  SetEnumDescr<int>(fourCCs, "YUY2", FOURCC(YUY2), "YUV 4:2:2 as for UYVY but with different component ordering within the u_int32 macropixel.", isOnlyCodec(Codec::Jpeg));

  fourCCs["NV24"] = { FOURCC(NV24), "YUV file contains 4:4:4 8-bit video samples stored in semi-planar format with all picture luma (Y) samples followed by interleaved U and V chroma sample.", aomituCodecs() };
  fourCCs["I444"] = { FOURCC(I444), "YUV file contains 4:4:4 8-bit video samples stored in planar format with all picture luma (Y) samples followed by chroma samples (all U samples then all V samples).", aomituCodecs() };
  fourCCs["T548"] = { FOURCC(T548), "YUV file contains 4:4:4 8-bit video samples stored in 32x4 tiles", allCodecs() };
  fourCCs["T648"] = { FOURCC(T648), "YUV file contains 4:4:4 8-bit video samples stored in 64x4 tiles", allCodecs() };
  SetEnumDescr<int>(fourCCs, "NV24", FOURCC(NV24), "YUV file contains 4:4:4 8-bit video samples stored in semi-planar format with all picture luma (Y) samples followed by interleaved U and V chroma sample.", isOnlyCodec(Codec::Jpeg));
  SetEnumDescr<int>(fourCCs, "I444", FOURCC(I444), "YUV file contains 4:4:4 8-bit video samples stored in planar format with all picture luma (Y) samples followed by chroma samples (all U samples then all V samples).", isOnlyCodec(Codec::Jpeg));

  fourCCs["Y010"] = { FOURCC(Y010), "YUV file contains 10-bit monochrome samples.", allCodecs() };
  fourCCs["T5mA"] = { FOURCC(T5mA), "YUV file contains 4:0:0 10-bit video samples stored in 32x4 tiles", allCodecs() };
  fourCCs["T6mA"] = { FOURCC(T6mA), "YUV file contains 4:0:0 10-bit video samples stored in 64x4 tiles", allCodecs() };
  ...
  fourCCs["P016"] = { FOURCC(P016), "YUV file contains 4:2:0 10/12/16-bit video samples each stored in a 16-bit MSB word in semi-planar format with all picture luma (Y) samples followed by chroma samples (all U samples then all V samples).", allCodecs() };
  fourCCs["P216"] = { FOURCC(P216), "YUV file contains 4:2:2 10/12/16-bit video samples each stored in a 16-bit MSB word in semi-planar format with all picture luma (Y) samples followed by chroma samples (all U samples then all V samples).", allCodecs() };
  fourCCs["Y416"] = { FOURCC(Y416), "YUV file contains 4:4:4 10/12/16-bit video samples each stored in a 16-bit MSB word in planar format with all picture luma (Y) samples followed by chroma samples (all U samples then all V samples).", allCodecs() };

  fourCCs["T50A"] = { FOURCC(T50A), "YUV file contains 4:2:0 10-bit video samples stored in 32x4 tiles", allCodecs() };
  fourCCs["T60A"] = { FOURCC(T60A), "YUV file contains 4:2:0 10-bit video samples stored in 64x4 tiles", allCodecs() };
  ...
  fourCCs["T52A"] = { FOURCC(T52A), "YUV file contains 4:2:2 10-bit video samples stored in 32x4 tiles", allCodecs() };
  fourCCs["T62A"] = { FOURCC(T62A), "YUV file contains 4:2:2 10-bit video samples stored in 64x4 tiles", allCodecs() };
  fourCCs["I4AL"] = { FOURCC(I4AL), "YUV file contains 4:4:4 10-bit video samples each stored in a 16-bit word in planar format with all picture luma (Y) samples followed by chroma samples (all U samples then all V samples).", allCodecs() };
  fourCCs["P410"] = { FOURCC(P410), "YUV file contains 4:4:4 10-bit video samples each stored in a 16-bit word in semi-planar format with all picture luma (Y) samples followed by interleaved U and V chroma samples.", allCodecs() };
  fourCCs["T54A"] = { FOURCC(T54A), "YUV file contains 4:4:4 10-bit video samples stored in 32x4 tiles", allCodecs() };
  fourCCs["T64A"] = { FOURCC(T64A), "YUV file contains 4:4:4 10-bit video samples stored in 64x4 tiles", allCodecs() };

  fourCCs["Y012"] = { FOURCC(Y012), "YUV file contains 12-bit monochrome samples.", allCodecs() };
  fourCCs["T5mC"] = { FOURCC(T5mC), "YUV file contains 4:0:0 12-bit video samples stored in 32x4 tiles", allCodecs() };
  fourCCs["T6mC"] = { FOURCC(T6mC), "YUV file contains 4:0:0 12-bit video samples stored in 64x4 tiles", allCodecs() };
  ...
  fourCCs["T50C"] = { FOURCC(T50C), "YUV file contains 4:2:0 12-bit video samples stored in 32x4 tiles", allCodecs() };
  fourCCs["T60C"] = { FOURCC(T60C), "YUV file contains 4:2:0 12-bit video samples stored in 64x4 tiles", allCodecs() };
  fourCCs["I2CL"] = { FOURCC(I2CL), "YUV file contains 4:2:2 12-bit video samples each stored in a 16-bit word in planar format with all picture luma (Y) samples followed by chroma samples (all U samples then all V samples).", aomituCodecs() };
  fourCCs["P212"] = { FOURCC(P212), "YUV file contains 4:2:2 12-bit video samples each stored in a 16-bit word in semi-planar format with all picture luma (Y) samples followed by interleaved U and V chroma samples.", aomituCodecs() };
  fourCCs["T52C"] = { FOURCC(T52C), "YUV file contains 4:2:2 12-bit video samples stored in 32x4 tiles", allCodecs() };
  fourCCs["T62C"] = { FOURCC(T62C), "YUV file contains 4:2:2 12-bit video samples stored in 64x4 tiles", allCodecs() };
  SetEnumDescr<int>(fourCCs, "I2CL", FOURCC(I2CL), "YUV file contains 4:2:2 12-bit video samples each stored in a 16-bit word in planar format with all picture luma (Y) samples followed by chroma samples (all U samples then all V samples).", isOnlyCodec(Codec::Jpeg));
  SetEnumDescr<int>(fourCCs, "P212", FOURCC(P212), "YUV file contains 4:2:2 12-bit video samples each stored in a 16-bit word in semi-planar format with all picture luma (Y) samples followed by interleaved U and V chroma samples.", isOnlyCodec(Codec::Jpeg));
  fourCCs["I4CL"] = { FOURCC(I4CL), "YUV file contains 4:4:4 12-bit video samples each stored in a 16-bit word in planar format with all picture luma (Y) samples followed by chroma samples (all U samples then all V samples).", aomituCodecs() };
  fourCCs["T54C"] = { FOURCC(T54C), "YUV file contains 4:4:4 12-bit video samples stored in 32x4 tiles", allCodecs() };
  fourCCs["T64C"] = { FOURCC(T64C), "YUV file contains 4:4:4 12-bit video samples stored in 64x4 tiles", allCodecs() };
  SetEnumDescr<int>(fourCCs, "I4CL", FOURCC(I4CL), "YUV file contains 4:4:4 12-bit video samples each stored in a 16-bit word in planar format with all picture luma (Y) samples followed by chroma samples (all U samples then all V samples).", isOnlyCodec(Codec::Jpeg));
  return fourCCs;
}
...
static void populateInputSection(ConfigParser& parser, ConfigFile& cfg)
{
  ...
  parser.addPath(curSection, "GMVFile", cfg.sGMVFileName, "optional input file for Global Motion Vector feature. When this parameter is not present, the GMV feature is disabled. The GMV file format is specified in file format page in the references section", aomituCodecs());
  parser.addNote(curSection, "GMVFile", "For more details on file structure, see :ref:`File formats page<tests_application/encoder/user_manuals/file_formats-label>`");
  ...
  sCropAlignX << HW_IP_BURST_ALIGNMENT;
  sCropAlignX << " in raster 8bit, " << (HW_IP_BURST_ALIGNMENT / sizeof(uint16_t)) << " in raster 10b";
  sCropAlignX << "/12b";
  sCropAlignX << ", 32 in tile 32x4 or 64 in tile 64x4";
  sCropAlignY << "2";
...
static void populateRCParam(Section curSection, ConfigParser& parser, AL_TRCParam& RCParam)
{
  ...
  parser.addArithFunc<decltype(RCParam.uMaxPSNR), double>(curSection, "MaxPSNR", RCParam.uMaxPSNR, [](double value)
  {
    return (decltype(RCParam.uMaxPSNR))(value * 100);
...
static void populateGopSection(ConfigParser& parser, ConfigFile& cfg)
{
  ...
  parser.addBool(curSection, "Gop.DoubleRef", GopParam.bDoubleRef, "Allows a picture P to reference two different pictures with half-search range for each reference picture", allCodecs());
...
static void populateProfileAndLevel(ConfigParser& parser, ConfigFile& cfg, Section& curSection)
{
  ...
  profiles["AVC_CAVLC_444_INTRA"] = { AL_PROFILE_AVC_CAVLC_444_INTRA, "See AVC/H.264 specification", isOnlyCodec(Codec::Avc) };
  profiles["AVC_CAVLC_444"] = { AL_PROFILE_AVC_CAVLC_444_INTRA, "DEPRECATED: use AVC_CAVLC_444_INTRA instead", isOnlyCodec(Codec::Avc) };
...
static void populateSettingsSection(ConfigParser& parser, ConfigFile& cfg, Temporary& temp, std::ostream& warnStream)
{
  ...
  SetEnumDescr<int>(chromaModes, "CHROMA_4_4_4", AL_CHROMA_4_4_4, "The stream is encoded with 4:4:4 chroma subsampling", aomituCodecs());
  ...
  parser.addFlag(curSection, "UseL2C", cfg.Settings.tChParam[0].eEncOptions, AL_OPT_L2_CACHE, "Specifies if the encoder should use the L2 cache or not", aomituCodecs());
  parser.addFlag(curSection, "PCM", cfg.Settings.tChParam[0].eEncTools, AL_OPT_PCM, "Specifies if the encoder should consider the PCM tool or not", filterCodecs({ Codec::Avc, Codec::Hevc, Codec::Vvc }));
  parser.addFlag(curSection, "Compression", cfg.Settings.tChParam[0].eEncOptions, AL_OPT_COMPRESS, "Specifies if the Frame Buffer Compression of references and/or reconstructed frames should be used", aomituCodecs());
  map<string, EnumDescription<int>> srcformats;
  srcformats["NVX"] = { AL_SRC_FORMAT_RASTER, "DEPRECATED, use RASTER instead", allCodecs() };
  srcformats["RASTER"] = { AL_SRC_FORMAT_RASTER, "Source mode raster", allCodecs() };
  srcformats["RASTER_MSB"] = { AL_SRC_FORMAT_RASTER_MSB, "Source mode raster MSB", allCodecs() };
  srcformats["TILE_64x4"] = { AL_SRC_FORMAT_TILE_64x4, "Source mode tile: 64x4", allCodecs() };
  srcformats["TILE_32x4"] = { AL_SRC_FORMAT_TILE_32x4, "Source mode tile: 32x4", allCodecs() };
  parser.addEnum(curSection, "SrcFormat", cfg.eSrcFormat, srcformats, "Specifies the Hardware Source buffer format");
  ...
  map<string, EnumDescription<int>> weighted_pred;
  weighted_pred["DEFAULT"] = { AL_WP_DEFAULT, "Default Weighted Prediction", isOnlyCodec(Codec::Avc) };
  weighted_pred["IMPLICIT"] = { AL_WP_IMPLICIT, "Implicit Weighted Prediction", isOnlyCodec(Codec::Avc) };
  parser.addEnum(curSection, "WeightedPred", cfg.Settings.tChParam[0].eWPMode, weighted_pred, "Specify the weighted prediction mode applied on bi-predictor blocks");
  parser.addFlag(curSection, "DirectSpatial", cfg.Settings.tChParam[0].eEncTools, AL_OPT_DIRECT_SPATIAL, "DEPRECATED, use DirectMode instead", isOnlyCodec(Codec::Avc));

  map<string, EnumDescription<int>> direct_mode;
  direct_mode["TEMPORAL"] = { 0, "Temporal direct mode", isOnlyCodec(Codec::Avc) };
  direct_mode["SPATIAL"] = { AL_OPT_DIRECT_SPATIAL, "Spatial direct mode", isOnlyCodec(Codec::Avc) };

  parser.addFlagsFromEnum(curSection, "DirectMode", cfg.Settings.tChParam[0].eEncTools, direct_mode, AL_OPT_DIRECT_SPATIAL, "DirectMode exploits the spatial or temporal correlation on B Frames by deriving its motion vector from previously encoded information.");

  populateMultipassOptions(parser, cfg, curSection);

  parser.addBool(curSection, "SrcSync", cfg.Settings.tChParam[0].bSrcSync, "Use synchronised source for encoder", allCodecs());
  parser.addArith(curSection, "SrcSyncChan", cfg.Settings.tChParam[0].uSrcSyncChanID, "Use synchronised source channel ID", {
...
static void populateRunSection(ConfigParser& parser, ConfigFile& cfg)
{
  ...
  parser.addBool(curSection, "EmulateSyncSrc", cfg.RunInfo.bEmulateSrcSync, "Emulate the Synchronised source directly from the encoder", allCodecs());
}
...
static void UpdateRelativePathFiles(string const& cfgFilename, ConfigFile& cfg)
{
  ...
  CheckAndUpdateRelativePath(cfgPath, cfg.sGMVFileName);
