...
typedef struct AL_IEncArchVtable
{
  ...
  AL_ERR (* EncoderCreateWithCtx)(AL_HEncoder* hEnc, AL_RiscV_Ctx ctx, AL_TAllocator* pAlloc, AL_TEncSettings const* pSettings, AL_CB_EndEncoding callback);
  ...
  void (* EncoderNotifyIsSkip)(AL_HEncoder hEnc);
  void (* EncoderNotifyGMV)(AL_HEncoder hEnc, int iNumFrame, int GMV_x, int GMV_y);
  ...
  bool (* EncoderSetAutoQPThresholdAndDelta)(AL_HEncoder hEnc, bool bUserAutoQP, AL_TAutoQPCtrl* userAutoQPValues);
