...
/***************************************************************************/
void AL_Common_Encoder_NotifyIsSkip(AL_TEncCtx* pCtx)
{
  AL_TEncRequestInfo* pReqInfo = getCurrentCommands(&pCtx->tLayerCtx[0]);
  pReqInfo->eReqOptions |= AL_OPT_IS_SKIP;
}

/***************************************************************************/
void AL_Common_Encoder_NotifyGMV(AL_TEncCtx* pCtx, int iNumFrame, int GMV_x, int GMV_y)
{
  AL_TEncRequestInfo* pReqInfo = getCurrentCommands(&pCtx->tLayerCtx[0]);
  pReqInfo->eReqOptions |= AL_OPT_USE_GMV;
  pReqInfo->gmv.iFrameNum = iNumFrame;
  pReqInfo->gmv.GMV_x = GMV_x;
  pReqInfo->gmv.GMV_y = GMV_y;
}
...
void AL_Common_Encoder_SetEncodingOptions(AL_TEncCtx* pCtx, AL_TFrameInfo* pFI, int iLayerID)
{
  ...
  if(pCtx->pSettings->tChParam[iLayerID].eEncOptions & AL_OPT_L2_CACHE)
...
bool AL_Common_Encoder_Process(AL_TEncCtx* pCtx, AL_TBuffer* pFrame, AL_TBuffer* pQpTable, int iLayerID)
{
  ...
  if(pQpTable != NULL && pCtx->pSettings->bDiagnostic)
  {
    ...
    int iQPTableDepth = 0;
    iQPTableDepth = pCtx->pSettings->tChParam[iLayerID].iQPTableDepth;
  ...
  addresses.tSrcAddrs.pC2 = 0;

  if(AL_PLANE_MODE_PLANAR == tPicFormat.ePlaneMode)
  {
    addresses.tSrcAddrs.pC1 = AL_PixMapBuffer_GetPlanePhysicalAddress(pFrame, AL_PLANE_U);
    addresses.tSrcAddrs.pC2 = AL_PixMapBuffer_GetPlanePhysicalAddress(pFrame, AL_PLANE_V);
  }
  else
  ...
  if(pChParam->bEnableSrcCrop)
  {
    ...
    if(AL_PLANE_MODE_PLANAR == tPicFormat.ePlaneMode)
    {
      if(AL_GET_CHROMA_MODE(pChParam->ePicFormat) != AL_CHROMA_4_4_4)
        iPosX /= 2;
      addresses.tSrcAddrs.pC1 += iPosY * AL_PixMapBuffer_GetPlanePitch(pFrame, AL_PLANE_U) + iPosX;
      addresses.tSrcAddrs.pC2 += iPosY * AL_PixMapBuffer_GetPlanePitch(pFrame, AL_PLANE_V) + iPosX;
    }
    else
  ...
  {
    addresses.tSrcAddrs.pMapY = AL_PixMapBuffer_GetPlanePhysicalAddress(pFrame, AL_PLANE_MAP_Y);
    addresses.tSrcAddrs.pMapC1 = 0;
    addresses.tSrcAddrs.pMapC2 = 0;

    if(AL_PLANE_MODE_PLANAR == tPicFormat.ePlaneMode)
    {
      addresses.tSrcAddrs.pMapC1 = AL_PixMapBuffer_GetPlanePhysicalAddress(pFrame, AL_PLANE_MAP_U);
      addresses.tSrcAddrs.pMapC2 = AL_PixMapBuffer_GetPlanePhysicalAddress(pFrame, AL_PLANE_MAP_V);
    }
    else

    if(AL_PLANE_MODE_SEMIPLANAR == tPicFormat.ePlaneMode)
      addresses.tSrcAddrs.pMapC1 = AL_PixMapBuffer_GetPlanePhysicalAddress(pFrame, AL_PLANE_MAP_UV);
...
void AL_Common_Encoder_ComputeRCParam(int iCbOffset, int iCrOffset, int iIntraOnlyOff, AL_TEncChanParam* pChParam)
{
  ...
  int iMinQP = 0;
  int iMaxQP = AL_IS_HEVC(pChParam->eProfile) ? 63 : 51;
...
/****************************************************************************/
bool AL_Common_Encoder_SetAutoQPThresholdAndDelta(AL_TEncoder* pEnc, bool bUserAutoQP, AL_TAutoQPCtrl* userAutoQPValues)
{
  AL_TEncCtx* pCtx = pEnc->pCtx;

  // safety check on values
  if(bUserAutoQP)
  {
    // thresholdQP values have to increase
    for(int i = 1; i < AL_QP_CTRL_MAX_NUM_THRESHOLDS; i++)
    {
      if(userAutoQPValues->thresholdQP[i - 1] > userAutoQPValues->thresholdQP[i] && userAutoQPValues->thresholdQP[i - 1] != INT32_MAX)
        AL_RETURN_ERROR(AL_ERR_INVALID_CMD_VALUE);
    }

    // deltaQP values needs to be in the correct bounds
    for(int i = 0; i < AL_QP_CTRL_MAX_NUM_DELTA_QPS; i++)
    {

      if(AL_IS_AVC(pEnc->pCtx->pSettings->tChParam[0].eProfile) && (userAutoQPValues->deltaQP[i] > 51 || userAutoQPValues->deltaQP[i] < -51))
        AL_RETURN_ERROR(AL_ERR_INVALID_CMD_VALUE);
  ...
  {
    AL_TEncRequestInfo* pReqInfo = getCurrentCommands(&pCtx->tLayerCtx[i]);
    pReqInfo->eReqOptions |= AL_OPT_UPDATE_AUTO_QP_VALUES;
    pCtx->pSettings->tChParam[i].bUserAutoQP = bUserAutoQP;
    pReqInfo->smartParams.bUserAutoQP = bUserAutoQP;
    Rtos_Memcpy(&pCtx->pSettings->tChParam[i].sUserAutoQPValues, userAutoQPValues, sizeof(AL_TAutoQPCtrl));
    Rtos_Memcpy(&pReqInfo->smartParams.userAutoQPValues, userAutoQPValues, sizeof(AL_TAutoQPCtrl));
  }
