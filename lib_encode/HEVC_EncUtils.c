...
void AL_HEVC_GenerateSPS(AL_TSps* pISPS, AL_TEncSettings const* pSettings, AL_TEncChanParam const* pChParam, int iMaxRef, int iCpbSize, int iLayerId)
{
  ...
  pSPS->sample_adaptive_offset_enabled_flag = (pChParam->eEncTools & AL_OPT_SAO) ? 1 : 0;
  pSPS->pcm_enabled_flag = (pChParam->eEncTools & AL_OPT_PCM) ? 1 : 0;
  ...
  if(pSPS->pcm_enabled_flag)
  {
    ...
    pSPS->pcm_loop_filter_disabled_flag &= pSPS->sample_adaptive_offset_enabled_flag ? 0 : 1;
  }
...
void AL_HEVC_GeneratePPS(AL_TPps* pIPPS, AL_TEncSettings const* pSettings, AL_TEncChanParam const* pChParam, int iMaxRef, int iLayerId)
{
  ...
  bSaoScaleNeeded = AL_GET_BITDEPTH(pChParam->ePicFormat) > 10 && ((pChParam->eEncTools & AL_OPT_SAO) ? 1 : 0);
